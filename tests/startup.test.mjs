...
function startSession(outcomes=['played']){
  ...
  const xp={$, $$:selector=>selector.startsWith('.welcome-user')?[$('.welcome-user')]:[],esc:String,icon:()=>'',avatar:()=>'',avatarPath:()=>'',recycleIcon:()=>'recycle',accounts:()=>[{id:'admin',name:'Test',avatar:'chess',type:'admin',active:true,enabled:true}],switchUser:()=>false,state:{files:[],iconPositions:{},user:'Test',showWelcome:false},register(name,fn){apps.set(name,fn);},persist(){},notify(title){notices.push(title);},hideMenus(){},applySettings(){},windows:new Map(),open(name){return apps.get(name)?.();},createWindow(){lastWindow={el:element(),body:element(),cleanup:[],close(){}};return lastWindow;},sound(name){sounds.push({name,time:now});const result=name==='startup'?(outcomes.shift()||'played'):'played';return typeof result==='function'?result():Promise.resolve(result);}};
