...
(() => {
  ...
  XP.register('player',()=>{
    ...
    XP.menubar(w,{
      ...
      [t('Eszközök')]:[{label:t('Hangok és audioeszközök…'),action:()=>XP.open('sounds')},{label:t('Fájl tulajdonságai'),action:()=>{const track=tracks[current];XP.dialog(t('Médiafájl tulajdonságai'),`Cím: ${track.title}\nElőadó: ${track.artist}\nAlbum: ${track.album}\nIdőtartam: ${duration(track.duration)}\nTípus: ${track.video?t('Videó'):'Hang'}`);}}],
    ...
    shell.innerHTML=`<aside class="wmp-sidebar"><div class="wmp-brand">${icon('player')}<span>Windows<br><b>Media Player</b><small>9 SERIES</small></span></div><nav aria-label="Media Player nézetek">${tabs.map(([id,label])=>`<button data-mode="${id}"><span>›</span>${label}</button>`).join('')}</nav><button class="wmp-open xp-button" data-open-media>${esc(t('Fájl megnyitása…'))}</button></aside><section class="wmp-main"><header class="wmp-heading"><b>${esc(t('Most játszott'))}</b><span>Windows Media Player</span></header><div class="wmp-now"><div class="wmp-screen"><canvas aria-label="Zenei vizualizáció"></canvas><div class="wmp-idle">${icon('player')}<span>Windows Media Player<small>9 SERIES</small></span></div><div class="wmp-screen-caption"></div></div><aside class="wmp-list"><header>${esc(t('Lejátszási lista'))}</header><div class="wmp-track-list"></div></aside></div><div class="wmp-page" hidden></div></section>`;
    ...
    function show(value){
      ...
      if(mode==='guide')page.innerHTML=`<div class="wmp-guide"><h1>${esc(t('Üdv a Műsorfüzetben!'))}</h1><p>${esc(t('Zene, videó és a kedvenc lejátszási listáid.'))}</p><h2>A Windows hangjai</h2><p>${esc(t('Ismerős dallamok egy ismerős asztalról. Hallgasd meg a Windows XP rendszerhangjait!'))}</p><button class="xp-button" data-system-sounds>${esc(t('Lejátszás'))}</button><h2>${esc(t('A saját zenéid'))}</h2><p>${esc(t('Nyisd meg a gépeden tárolt zenéket és videókat, majd állíts össze egy listát.'))}</p><button class="xp-button" data-open-media>${esc(t('Médiafájlok megnyitása…'))}</button><hr><button class="web-link" data-music-site>${esc(t('Zeneszoba – albumok és zenetörténet'))}</button></div>`;
      if(mode==='radio')page.innerHTML=`<div class="wmp-guide"><h1>${esc(t('Rádióállomások'))}</h1><p>${esc(t('Rendszerezd az állomásokat, és fedezd fel a ZeneSzoba műsorfüzetét.'))}</p><table class="wmp-library"><tr><th>${esc(t('Állomás'))}</th><th>${esc(t('Műfaj'))}</th><th>${esc(t('Állapot'))}</th></tr><tr><td>Retro FM</td><td>${esc(t('Slágerek'))}</td><td>${esc(t('Az adás nem érhető el'))}</td></tr><tr><td>${esc(t('Jazz Café'))}</td><td>Jazz</td><td>${esc(t('Az adás nem érhető el'))}</td></tr><tr><td>Classic Radio</td><td>Klasszikus</td><td>${esc(t('Az adás nem érhető el'))}</td></tr></table><p><button class="xp-button" data-music-site>${esc(t('Műsorfüzet megnyitása'))}</button></p></div>`;
      if(mode==='rip'||mode==='burn')page.innerHTML=`<div class="wmp-disc">${icon('cd')}<h2>${mode==='rip'?t('Másolás CD-ről'):t('Másolás CD-re vagy eszközre')}</h2><label>${esc(t('Meghajtó:'))} <select aria-label="CD-meghajtó"><option>${esc(t('DVD-meghajtó (D:)'))}</option></select></label><p class="wmp-disc-status">Helyezzen be egy ${mode==='rip'?'zenei':t('írható')} lemezt a D: meghajtóba.</p><button class="xp-button" data-disc-refresh>${esc(t('Frissítés'))}</button>${mode==='burn'?`<p>A médiatárban `+tracks.length+` fájl vár lejátszásra.</p>`:''}</div>`;
      if(mode==='now')draw();
    }
