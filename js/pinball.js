'use strict';
(() => {
  const {register,createWindow,menubar,state,$}=XP;
  register('pinball',()=>{
    ...
    menubar(w,{
      ...
      'Beállítások':()=>[{label:'Hangeffektusok',checked:!muted,action:sound}],
      'Súgó':[{label:'Irányítás és játékszabályok',action:help},{label:'A Space Cadetről',action:()=>XP.dialog('3D Pinball – Space Cadet','Az eredeti Space Cadet böngészős portja.\nJátékmotor: k4zmu2a / alula, MIT licenc.\nWebAssembly-csomag: Luciano Russo.\nEredeti játék: Cinematronics / Maxis / Microsoft.\n\nA játék minden szükséges fájlja helyben van.') }]
    });
    ...
    const status=XP.status(w,'A Space Cadet betöltése…').firstElementChild;
