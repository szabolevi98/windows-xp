...
register('calendar',()=>{
 ...
 sheet=propertySheet({app:'calendar',title:t("text_date_and_time_properties"),icon:'datetime',width:440,height:500,initial:'datetime',tabs:[['datetime',t("text_date_and_time")],['timezone',t("text_time_zone")],['internet',t("text_internet_time")]],
  ...
  draw(tab,panel){
   ...
   if(tab==='internet')panel.innerHTML=`<fieldset><legend>${esc(t("text_internet_time"))}</legend><label class="settings-check"><input type="checkbox" name="internet-time" ${draft.internet?'checked':''}> ${esc(t("text_automatically_synchronize_with_an_internet_time_server"))}</label><label class="settings-field"><span>${esc(t("text_server"))}</span><select disabled><option>time.windows.com</option></select></label><button class="xp-button" data-sync>${esc(t("text_update_now"))}</button><p class="settings-note">${esc(t("text_time_sync_is_unavailable_while_offline"))}</p></fieldset>`;$('[data-sync]',panel).onclick=()=>XP.dialog(t("text_internet_time"),t("text_time_sync_is_unavailable_while_offline"),{icon:'error'});
  },
