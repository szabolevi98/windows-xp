...
register('player',()=>{
 if(XP.singleton('player'))return;const w=createWindow({title:'Windows Media Player',icon:'player',app:'player',className:'player-window',width:680,height:500,minWidth:430,minHeight:330});
 ...
 const tabs=[['now','Most játszott'],['library','Médiatár'],['guide','Műsorfüzet'],['open','Fájl megnyitása']];
 const body=document.createElement('div');body.className='player-body';body.innerHTML=`<aside class="player-sidebar">${tabs.map(([key,label])=>`<button class="${key==='now'?'active':''}" data-player-tab="${key}"><b></b>${esc(label)}</button>`).join('')}</aside><div class="player-main"><header class="player-bar"><span class="player-mode">Most játszott</span><span class="player-watermark">Windows Media Player</span></header><div class="player-panes"><div class="player-visual"><div class="visual-bars">${Array.from({length:29},(_,i)=>`<i style="--h:${18+Math.sin(i*.6)**2*78}px;--delay:${-i*.12}s"></i>`).join('')}</div></div><div class="player-playlist"><div class="playlist-head">Lejátszási lista</div><div class="track-list"></div></div></div></div>`;w.body.append(body);
 const controls=document.createElement('div');controls.className='player-controls';controls.innerHTML='<div class="player-progress"><input type="range" min="0" max="1000" value="0" aria-label="Lejátszási pozíció"><span class="player-time">00:00 / 00:00</span></div><div class="player-buttons"><button data-player="play" class="play" aria-label="Lejátszás" title="Lejátszás"></button><button data-player="stop" aria-label="Leállítás" title="Leállítás"></button><button data-player="prev" aria-label="Előző" title="Előző"></button><button data-player="next" aria-label="Következő" title="Következő"></button><span class="player-track-name"></span><span class="player-volume"><i></i><input type="range" min="0" max="100" aria-label="Lejátszó hangereje"></span></div>';w.body.append(controls);
 const format=t=>Number.isFinite(t)?`${String(Math.floor(t/60)).padStart(2,'0')}:${String(Math.floor(t%60)).padStart(2,'0')}`:'00:00';
 function renderList(){$('.track-list',body).innerHTML=tracks.map((t,i)=>`<button class="track-row ${i===index?'active':''}" data-track="${i}"><span>${i+1}</span><b>${esc(t.title)}</b><small>${esc(t.artist)}</small></button>`).join('');$('.playlist-head',body).textContent=`Lejátszási lista (${tracks.length})`;}
 ...
 function sync(){const playing=!audio.paused;$('.player-visual',body).classList.toggle('playing',playing);const play=$('[data-player=play]',controls);play.classList.toggle('playing',playing);play.title=play.ariaLabel=playing?'Szünet':'Lejátszás';$('.player-time',controls).textContent=`${format(audio.currentTime)} / ${format(audio.duration)}`;$('.player-progress input',controls).value=audio.duration?audio.currentTime/audio.duration*1000:0;}
