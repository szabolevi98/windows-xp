...
function settings(initial='desktop'){
 ...
 if(current==='system'){const used=new Blob([JSON.stringify(state)]).size;panel.innerHTML=`<div class="system-brand">${icon('windows')}<div><strong>Windows<span>xp</span></strong><br>Professional</div></div><dl class="system-facts"><dt>Rendszer:</dt><dd>Microsoft Windows XP<br>Version 2002<br>Service Pack 3</dd><dt>Regisztrált felhasználó:</dt><dd>${esc(state.user)}</dd><dt>Kiadás:</dt><dd>Böngészős szimulátor<br>HTML + JavaScript</dd><dt>Helyben tárolt adatok:</dt><dd>${(used/1024).toFixed(1)} KB<br>${state.files.filter(f=>!f.deleted).length} saját elem</dd></dl><p class="settings-note">A fájlok és beállítások a böngésző helyi tárhelyén maradnak. A fontos dokumentumokat a Fájl menüből le is töltheted.</p>`;}
 }
...
register('network',()=>{
 if(XP.singleton('network'))return;const w=createWindow({title:'Hálózati kapcsolatok',icon:'network',app:'network',width:530,height:330});w.body.innerHTML=`<div class="network-body"><h2 style="font-size:16px;color:#214f98">Helyi kapcsolat</h2><div class="network-connection">${icon('network')}<div><b>Csatlakoztatva a helyi webhez</b><br>Sebesség: 100,0 Mbps (szimulált)<br>Állapot: helyi bemutatóhálózat</div></div><p>Az Internet Explorer a számítógépen tárolt oldalakat jeleníti meg. A kereséseid és a dokumentumaid ezen a gépen maradnak.</p><button class="xp-button" data-open="ie">Böngésző megnyitása</button></div>`;return w;
});
...
register('security',()=>{
 ...
 const parts=()=>[
  ...
  {id:'virus',name:'Vírusvédelem',on:null,
   bad:'A Windows nem talált vírusvédelmi programot ezen a számítógépen. Ebben a szimulátorban nincs is mit telepíteni: a gép nem csatlakozik hálózathoz, és nem futtat idegen programot.'}
 ];
