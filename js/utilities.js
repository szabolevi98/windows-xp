...
function userAccounts(){
 ...
 const typeName=type=>type==='guest'?'Vendég fiók':type==='limited'?'Korlátozott fiók':'Számítógép rendszergazdája';
 ...
 const guestTile=()=>tile(guest(),'guest',guest().enabled?null:'A Vendég fiók ki van kapcsolva');
 ...
 body.onclick=e=>{
  ...
  if(button.dataset.guest){
   ...
   if(!XP.setGuest(on)){XP.sound('error');XP.dialog('Felhasználói fiókok','A Vendég fiókot csak a rendszergazda kapcsolhatja ki.',{icon:'error'});return;}
   notify('Felhasználói fiókok',on?'A Vendég fiók bekapcsolva. A bejelentkezési képernyőn már választható.':'A Vendég fiók kikapcsolva.');
  ...
  if(action==='name'){
   ...
   draftName=next;save();home();notify('Felhasználói fiókok',`A fiók új neve: ${next}`);return;
  }
