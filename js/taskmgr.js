...
(() => {
  ...
  const SYSTEM=[
    ...
    ['lsass.exe','SYSTEM',1284],['svchost.exe','SYSTEM',4784],['svchost.exe','HÁLÓZATI SZOLGÁLTATÁS',3496],
    ['svchost.exe','HELYI SZOLGÁLTATÁS',4256],['spoolsv.exe','SYSTEM',5140],['explorer.exe',null,14260]
  ];
  // Everything the machine is running: the services it always has, plus a process per open program.
  function processes(apps,user='Adminisztrátor'){
  ...
  XP.register('taskmgr',()=>{
    ...
    const rows=()=>processes(others().map(win=>win.app),state.user||'Adminisztrátor');

    XP.menubar(w,{
      'Fájl':()=>[{label:'Új feladat (Futtatás…)',action:()=>XP.open('run')},null,{label:'Kilépés a Feladatkezelőből',action:()=>w.close()}],
      'Beállítások':[{label:'Mindig látható',disabled:true},{label:'Kis méret használatkor',disabled:true}],
      'Nézet':()=>TABS.map(([key,label])=>({label,checked:tab===key,action:()=>{tab=key;render();}})),
      'Leállítás':[{label:'Készenlét',action:()=>XP.open('power')},{label:'Kikapcsolás',action:()=>XP.open('power')},null,{label:'Kijelentkezés',action:()=>XP.open('logoff')}],
      'Súgó':[{label:'A Feladatkezelő névjegye',action:()=>XP.dialog('Windows Feladatkezelő','Windows Feladatkezelő\n\nA futó programokat és folyamatokat mutatja, és le is állíthatod őket.\n\nMegnyitás: Ctrl+Shift+Esc vagy Ctrl+Alt+Del.')}]
    });
    ...
    function render(){
      ...
      if(tab==='apps'){
        panel.innerHTML=running.length
          ?`<table class="taskmgr-table"><thead><tr><th>Feladat</th><th>Állapot</th></tr></thead><tbody>${running.map(win=>
             `<tr class="${win.id===selectedTask?'selected':''}" data-task="${win.id}"><td>${icon(win.icon)}${esc(win.title)}</td><td>${win.minimized?'Fut':'Fut'}</td></tr>`).join('')}</tbody></table>`
          :'<p class="taskmgr-empty">Nincs futó alkalmazás.</p>';
        buttons.innerHTML='<button class="xp-button" data-do="end">Feladat befejezése</button><button class="xp-button" data-do="switch">Váltás</button><button class="xp-button" data-do="new">Új feladat…</button>';
      }
      if(tab==='processes'){
        panel.innerHTML=`<table class="taskmgr-table processes"><thead><tr><th>Képfájlnév</th><th>Felhasználónév</th><th>CPU</th><th>Memóriahasználat</th></tr></thead><tbody>${list.map((process,index)=>{
        ...
        buttons.innerHTML='<button class="xp-button" data-do="kill">Folyamat leállítása</button>';
      }
      if(tab==='performance'){
        panel.innerHTML=`<div class="taskmgr-meters"><div><h3>CPU-használat</h3><div class="meter-box"><b>${cpu}%</b></div></div><div class="taskmgr-graph"><h3>CPU-használat előzményei</h3><canvas class="cpu-graph"></canvas></div></div><dl class="taskmgr-facts"><dt>Leírók</dt><dd>${8214+list.length*37}</dd><dt>Szálak</dt><dd>${312+list.length*9}</dd><dt>Folyamatok</dt><dd>${list.length}</dd><dt>Fizikai memória összesen</dt><dd>523 760 KB</dd><dt>Fizikai memória szabad</dt><dd>${(268400-list.length*1800).toLocaleString('hu-HU')} KB</dd><dt>Véglegesített memória</dt><dd>${(146200+list.length*2400).toLocaleString('hu-HU')} KB</dd></dl>`;
        graph($('.cpu-graph',panel),history,'#26ff5c');
        buttons.innerHTML='';
      }
      if(tab==='network'){
        panel.innerHTML=`<div class="taskmgr-graph wide"><h3>Helyi kapcsolat</h3><canvas class="net-graph"></canvas></div><table class="taskmgr-table"><thead><tr><th>Adapter</th><th>Hálózat kihasználtsága</th><th>Kapcsolat sebessége</th><th>Állapot</th></tr></thead><tbody><tr><td>Helyi kapcsolat</td><td>${(net.at(-1)/10).toFixed(2)} %</td><td>100 Mbps</td><td>Működik</td></tr></tbody></table>`;
    ...
    buttons.onclick=event=>{
      ...
      if(action==='kill'){
        ...
        if(!process)return;
        if(!process.app){XP.sound('error');XP.dialog('Feladatkezelő','Ez egy rendszerfolyamat, és nem állítható le.',{icon:'error'});return;}
        XP.confirm('Feladatkezelő – figyelmeztetés',`Biztosan leállítod a(z) ${process.name} folyamatot?\n\nA nem mentett adatok elvesznek.`).then(answer=>{
